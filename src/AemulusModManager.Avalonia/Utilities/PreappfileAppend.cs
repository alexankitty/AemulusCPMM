...
namespace AemulusModManager
{
    public static class PreappfileAppend
    {
        ...
        public static void Validate(string path, string cpkLang)
        {
            ...
            if (File.Exists($@"{path}\data00007.pac"))
            {
                ...
                )
                {
                    ...
                    if (
                        File.Exists(
                            $@"{path}\data00007\{string.Join("\\", folders.Skip(idx + 1).ToArray())}"
                        )
                    )
            ...
            if (File.Exists($@"{path}\movie00003.pac"))
            {
                ...
                )
                {
                    ...
                    if (
                        File.Exists(
                            $@"{path}\movie00003\{string.Join("\\", folders.Skip(idx + 1).ToArray())}"
                        )
                    )
        ...
        public static void Append(string path, string cpkLang)
        {
            ...
            startInfo.CreateNoWindow = true;
            startInfo.FileName =
                $@"{Path.GetDirectoryName(Assembly.GetEntryAssembly().Location)}\Dependencies\preappfile\preappfile${(OperatingSystem.IsWindows() ? ".exe" : "")}";
            startInfo.WindowStyle = ProcessWindowStyle.Hidden;
            startInfo.UseShellExecute = false;
            if (
                Directory.Exists(
